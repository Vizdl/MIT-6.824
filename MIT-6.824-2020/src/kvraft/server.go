...
func (kv *KVServer) PutAppend(args *PutAppendArgs, reply *PutAppendReply) {
	...
	log.Printf("准备抢占kv.mu.Lock()")
	kv.mu.Lock()
	log.Printf("成功抢占kv.mu.Lock()")
	...
	log.Printf("准备释放kv.mu.Lock()")
	kv.mu.Unlock()
	log.Printf("成功释放kv.mu.Lock()")
...
func (kv *KVServer) ReadApplyCh (){
	for true {
		...
		if kv.rf.RaftStatus() == kv.me {
			log.Printf("第 %d 台服务器准备将应用消息同步到 leaderChancel内", kv.me)
			kv.leaderChancel <- message
			log.Printf("第 %d 台服务器成功将应用消息同步到 leaderChancel内", kv.me)
		}
		kv.mu.Lock()
